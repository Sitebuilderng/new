	...
	{if $item[4]=="prevMonth"}{assign var=str value="`$item[0]` `$prev_month` `$prev_month_year`"}{/if}{if $item[4]=="currentMonth"}{assign var=str value="`$item[0]` `$month` `$year`"}{/if}{if $item[4]=="nextMonth"}{assign var=str value="`$item[0]` `$next_month` `$next_month_year`"}{/if}
		...
		<td class="{if $smarty.now|date_format:"%Y"==$year and $smarty.now|date_format:"%B"==$month and $smarty.now|date_format:"%d"==$item[0]} current{/if} {if $smarty.foreach.loop1.iteration % 7 == 6 || $smarty.foreach.loop1.iteration % 7 == 0}weekend{/if} {$item[4]} {if $item[1]|@count>0}hasEvents{/if}" data-full-date="{if $item[4]=="prevMonth"}{assign var=str value="$item[0] $prev_month $prev_month_year"}{$str|date_format_locale:"%d %B %Y":$language}{/if}{if $item[4]=="currentMonth"}{assign var=str value="$item[0] $month $year"}{$str|date_format_locale:"%d %B %Y":$language}{/if}{if $item[4]=="nextMonth"}{assign var=str value="$item[0] $next_month $next_month_year"}{$str|date_format_locale:"%d %B %Y":$language}{/if}">
			...
			{foreach from=$item[1] item=item1 key=key2 name=loop2}
				<div class="event" id="{$item1[1]}">{if $item1[2]!=""}<a href="{if $item1[3]!="External Website"}{/if}{$item1[2]}{if $item1[3]!="External Website"}{/if}" target="_blank">{/if}{$item1[0]}{if $item1[2]!=""}</a>{/if}
					<a href="/actions/downloadevent/?dateend={if $item1[11]==""}{$str|date_format:"%d-%B-%Y"}{if $item1[6]!=""} {$item1[6]|htmlspecialchars}{else} 10:00 am{/if}{else}{$item1[12].end_timestamp|date_format:"%d-%B-%Y %I:%M %p"}{/if}&location=&summary=&link={$item1[2]}&description={$item1[0]}&datestart={$str|date_format:"%d-%B-%Y"}{if $item1[6]!=""} {$item1[6]|htmlspecialchars}{else} 09:00am{/if}" class='download-event'></a></div>
			{/foreach}
			{/if}
		</td>
