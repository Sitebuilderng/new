<?php
	...
	class Registrar extends Model {
		public $credentials;
		public function domainTaken($domain)
		{
			// Check if domain is available to register here.
			$nc_api = array( 'api_user' => $this->credentials['username'],
					'api_key' => $this->credentials['key'],
					'api_ip' => 'detect'
				);
			$sandbox = $this->credentials['sandbox']; 
			$nc = new Namecheap( $nc_api, $sandbox );
			if ( $nc->domainsCheck( $domain ) ) {
				// Domain is available, return false for this function
				return false;
			} else {
				// Domain is taken, return true
				return true;
			}
		}
		public function registerOrCancel($domain)
		{
			// Get all domains currently needing registration.
			$toregister = $this->query('SELECT * FROM websites WHERE domain_registered=0 AND domain_register=1');
			foreach ($toregister as $website) {
				$result = true; // Register domain with registrar here. $result = true when registered. 
				if ($result) {
					$durationInSeconds = 362*86400; // Set to renew 3 days before a year end.
					$expires = time()+$durationInSeconds;
					
					$update = $this->query("UPDATE websites SET domain_registered=1, domain_register=0, domain_expires=".$this->to_datetime($expires)." WHERE websitesid=".$website->websitesid)
				}
			}
			

			$tocancel = $this->query('SELECT * FROM websites WHERE domain_registered=1 AND domain_cancel=1');
			foreach ($toregister as $website) {
				// If registrar auto-renews, disable it here. 
				
				$update = $this->query("UPDATE websites SET domain_registered=0, domain_cancel=0 WHERE websitesid=".$website->websitesid); // Set domain_registered to 0 to prevent renewal
				
			}
			
			$torenew = $this->query('SELECT * FROM websites WHERE domain_expires < NOW() AND domain_registered=1');
			foreach ($torenew as $website) {
				$durationInSeconds = 362*86400; // Set to renew 3 days before a year end.
				$expires = time()+$durationInSeconds;
				
				$update = $this->query("UPDATE websites SET domain_expires=".$this->to_datetime($expires)." WHERE websitesid=".$website->websitesid)
			}
